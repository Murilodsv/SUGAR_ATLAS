...
! ---   direct access table, with linear interpolation
	...
	implicit none	
      ...
      real         tbmax            !Maximum temperature for plant growth and development
      real         tbleaf           !
      real         thour(24)        !
...
1000  CONTINUE 
     ...
     !!&  swroottyp,wiltpoint,rootradius,rootcoefa,rsw)                   ! NwRootExtr
      ...
      ! Initial dry mass [t ha-1] in planting date (using flat bed method)
      ! Assuming stalks within 1.5 kg and 2 meters, spreading 2/4 of this 
      ! stalks per m2 and the total water content in these stalks is 80% *P*
          
          w = (1.5 * 2./4.) / (rowsp/100.) * (10000./1000.) * .2                    
          ...
          path_out = 'C:\Murilo\DOUTORADO\WUR\SWAP_Sugarcanev2\
     &SWAP_Sugarcanev2\SWAP_Sugarcanev2'
          
      OPEN(302,FILE='Param_Set.OUT',
     &      STATUS='OLD',READONLY)
          
          READ (302,119) maxgl,tb,rue,sla,extcoef,sgpf,dpercoeff,sucmax,
     & srl,chustk,chupeak,chudec,chumat,popmat,poppeak,phyloc,mla
          
          
                              
119     FORMAT(16(F11.3,/),1F11.3)
          close(302)
     ...
     &     abs((z(node) - 0.5*dz(node))))
      ...
      epp = ptra                    !(cm/d)
      ch = (pleng * 100.) + llength !(cm) Used for Penmam-M. method for et0
...
2000      CONTINUE
      ...
      par = (rad * .5)/0.1E7       !(MJ/m2.d)
      epp = ptra                   !(cm/d)
      ch = (pleng * 100.) + llength!(cm) Used in Penmam-M. method
      
      !------------------------------------------------------------
      
      !Calculating degree-days using a unique tb
      !Should try to simulate soil temperature for population and root growth
          
		if (t_mean .GE. tb .and. t_mean .LE. tbmax) then
      ...
      !Plant development before emergence
          
          if (.not. flemerged) then
          
              !Assuming the primary shoot growth rate of 0.08 cm/DG (Keating et. al. 1999) *P*
		    shootdepth = plantdepth - diac * 0.08             
      ...
      !Plant growth and development after emergence
		
		tbleaf = tb !Could be different tb for leaf fenology
						
		if (t_mean .GE. tbleaf .and. t_mean .LE. tbmax) then			
              dileaf = (t_mean-tbleaf)			
          else 			
              dileaf = 0.! Degree-Days for leaf development              
          endif		

		diaclf = diaclf + dileaf	     
                 
          !Crop coeficient (Kc) - Based on EORATIO method         
          cf = kc_min + (eoratio - kc_min) * lai / lai_max
          cf = max(0.,cf)
          
          
          call pgs(swfacp,chustk,par,diac,t_mean,w,extcoef,pg,rue,
     & li,task,tmax,tmin,thour)     
		      
          endif
		          
          call parts(diac,chustk,chuem,lgpf,sgpf,rgpf,stalkgpf,
     &   flemerged)          
      ...
      if (diac .LT. 1000) then   ! A variation in RDEPTH should be computed dur the water stress - the higher WS, the deeper the root goes
        rdepth = (0.08 * diac) + plantdepth ! *P* = 0.08 cm oC-1 day-1 reference?
...
3000    continue    
      ...
      if(.not.flemerged) then          
          ...
          wr  = w * (1-max((shootdepth/plantdepth), 0.d0))
		wr  = MAX(wr,0.0)         
                    
      else
                    
          call waterstress(wr,swfacp,swface,srl,swcon1,swcon2,
     &swcon3,laythi,wpp,fcp,stp)
     ...
     & li,task,tmax,tmin,thour)
			...
			dw =  MAX(dw, 0.)
              ...
              if (diac .GT. chuem .AND. diac .LT. chupeak) then 
				dnstk = (poppeak/(chupeak-chuem)) * di !/ ROWSP				   ! initial tiller grow
          ...
          CALL lais(daynr,dileaf,diaclf,mla,chustk,dnstk,nstk,ln,dnleaf,
     & maxgl,swface,sla,dla,dleafdm,wl,dw,lgpf,rgpf,sgpf,ddeadlm,
   ...
   !           if (diac .GT. chustk .AND. diac .LT. chupeak) then 
			...
			!ELSEIF (diac .GT. (chumat)) THEN
			!	dnstk = 0.		            						               ! late stable tiller population
			!END IF           						               ! late stable tiller population
     ...
     & deadln,lntotal,phyloc,li,ddealla,cumla)
		...
		wsfresh   = wsdm    + wstkwat
          ...
          !leaf length to compute plant height (stalk + leaf) M. Vianna
          llength   = ((la/ln) * 10000)/(0.75 * 5) !mean leaf lenght (5 is a mean leaf width and 0.75 shape factor)
...
9           FORMAT(I4,4X,I3,4X,I3,4X,I3,6F8.1,4F8.2,9F8.2)
	...
	!************************************************************************
      ...
      subroutine waterstress(wr,swfacp,swface,srl,swcon1,swcon2,
     &swcon3, laythi,wpp,fcp,stp)
      Use Variables
	...
	SUBROUTINE pgs(swfacp,chustk,par,diac,t_mean,w,extcoef,pg,rue,
     & li,task,tmax,tmin,thour)
	...
	!Age factor to reduce the photosynthesis due to age of crop
		...
		!Calling thours for temperature stress on photosynthesis
		...
		pg = max(pg  / 100.,0.)              ! (converting to t / ha d)
	...
	SUBROUTINE PARTS(diac,chustk,chuem,lgpf,sgpf,rgpf,stalkgpf,flemerged)
	...
	!-----------------------------------------------------------------------  
		...
		real        stalkgpf
          ...
          else if (diac.gt.chustk) then
              
              sgpf     = 0.0d0 !sgpf parameter *P*
              stalkgpf = 0.88d0
              rgpf     = (1 - stalkgpf) * 0.3d0
              lgpf     = (1 - stalkgpf) * 0.7d0
                     
          endif 
                              
      fcheck = (rgpf + sgpf + lgpf + stalkgpf) - 1.0d0
 ...
 !     subroutine lais1(daynr,dileaf,diaclf,mla,chustk,dnstk,nstk,ln,
 !    & dnleaf,maxgl,swface,sla,dla,dleafdm,wl,dw,lgpf,rgpf,sgpf,ddeadlm,
 !    & deadln,lntotal,phyloc,li,ddealla,cumla,poppeak)
	!implicit none
	!!-----------------------------------------------------------------------  
	!	
	!	REAL ln
 ...
 !         
 !         real poppeak
 !         real shoot(int(poppeak),5) !noden, ln, la, diam, top heigth
 !         real maxgl
 !         real la(int(maxgl),int(poppeak)+1)
 !     
 !         
 !         
 !         
 ...
 !         
 !         dnleaf = 1/phyloc * dileaf
 !         dleafdm = lgpf * dw/nstk * 100 ! In [g shoot-1]         
 !         
 !         do i = 1,int(nstk)
 !                           
 !             shoot(i,2) = shoot(i,2) + dnleaf              
 !             
 !             do n = 1,int(shoot(i,2))
 !                 
 !             la(n,i) =  la(n,i) + (dleafdm  
 !                 
 !             end do
 !             
 !             
